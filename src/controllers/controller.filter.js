...
const hall = asyncHandler(async(req,res)=>{
    ...
    const {rows:report}= await db.query("select * from report where mail_id = $1 and hall_id = $2",[email,id])
    const report_data = report.length===0 ? "Report" :"Reported"
